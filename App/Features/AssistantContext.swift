...
func buildAssistantContext(_ api: AssistantAppState, now: Date = Date()) -> [String: AnyJSON] {
    ...
    // 5-second start must not decide when "your focus window" is) — inside
    // goldenHours, which needs the raw rows to place a runaway at its real start.
...
func buildVoiceInstructions(_ api: AssistantAppState) -> String {
    ...
    return "You are \(name)'s PERSONAL assistant in Unstuck — you know them (the profile facts in the state below are for planning around, not for saying) "
        ...
        // couldn't do fortnightly). This build executes intervalWeeks, so it
        // carries the every-N-weeks wording — verbatim the shared vectors'
        // prompts.voiceRepeatsRule (lib/recurrence-vectors.json) and web's
        // REPEATS_RULE; the registry's set_task_recurrence description says
        // the same.
